...
public class EmojiRegexUtil {
    ...
    private static final String speChat = "[`~!@#_$%^&*()+=|{}':;',\\[\\].<>/?~！@#￥%……&*（）— +|{}【】‘；：”“’。，、？‖]";

    /**
     * emoji正则表达式
     */
    public static String getFullEmojiRegex1() {
        return "(?:"
    ...
    public static InputFilter getInputFilter(boolean special) {
        final String str;
        if (special) {
            str = getFullEmojiRegex();
        } else {
            str = getFullEmojiRegex1();
        }
        return new InputFilter() {
            Pattern emoji = Pattern.compile(str,
