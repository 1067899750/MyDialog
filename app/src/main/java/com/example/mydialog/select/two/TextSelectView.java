...
import com.example.mydialog.select.one.Utils;
...
public class TextSelectView extends android.support.v7.widget.AppCompatTextView {
    ...
    private List<ShowLine> BreakText(int viewwidth, int viewheight) {
        ...
        while (getText().toString().length() > 0) {
            BreakResult breakResult = TextBreakUtil.BreakText(getText().toString(), viewwidth, 0, mPaint);
            ...
            setText(getText().toString().substring(breakResult.ChartNums));
        }
